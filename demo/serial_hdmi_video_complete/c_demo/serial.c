            ...
            if (f_h_flag == 1)  // 有帧头，判断帧尾，接收消息
            {
                if (f_t1_flag == 1) //有帧头，有帧尾1
                {
                    if (chrtmp[tnum] == Frame_Tail2)
                    {
                        ...
                        //sprintf(str1, "bash /home/pi/demo/serial_hdmi_video/c_demo/video.sh %d", chrtmp[2]);
                        //sprintf(str1, "bash playVideo.sh %d", chrtmp[2]);
                        
                        //sprintf(str2, "echo -e \"\nomxplayer -o hdmi /home/pi/%04d.mp4 --loop\n\"", chrtmp[2]);
                        //sprintf(str2, "echo \"\nomxplayer -o hdmi /home/pi/%04d.mp4 --loop\n\"", chrtmp[2]);
